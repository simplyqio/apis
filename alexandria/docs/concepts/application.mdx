...
Many [endpoints](/docs/concepts/endpoint) can be created for an application.

### Event

[Events](/docs/concepts/event) are directly published to an application. Because an application also holds the endpoint information, it is possible to publish events to an application and have them automatically routed to the correct endpoint.
...
* `type` - The type retry algorithm. The following types are supported:
  ...
  | ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
  | `base_exponential_backoff_with_deadline` | This is the default retry strategy. It will retry the request with an exponential backoff until the deadline is reached. If the deadline is reached, the request will fail. You can view the retry schedule [here](/docs/retries/retry-schedule). |
